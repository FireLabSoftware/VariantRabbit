...
## Version FC adds multi-item option for vlog (multiple items will be separarted by spaces)
## Version FH adds an option OneReference to vFastAtoDict that merges all subsequencers in a FastA file into a single sequence
## Version FM adds insurance that title width doesn't exceed graphic width
...
vsgid = ''
for i in range(24):
    vsgid += random.choice('ZXCVBNMASDFGHJKLQWERTYUIOPzxcvbnmasdfghjklqwertyuiop1234567890')
vshortnow = strftime("%m%d%y_%H%M",localtime())
if 'spawned' in sys.argv:
    vSpawned = True
    vspPos = sys.argv.index('spawned')
    vargv = sys.argv[:vspPos]
    vsgid = sys.argv[vspPos+1]
    vPID = sys.argv[vspPos+2][:200]
    vPMax = sys.argv[vspPos+3]
    vnow = sys.argv[vspPos+4]
    vshortnow = sys.argv[vspPos+5]
    vt0 = float(sys.argv[vspPos+6])
    vSpawnFunction = sys.argv[vspPos+7]
    vSpawnParams = sys.argv[vspPos+8:]
else:
    vSpawned = False
    vSpawnFunction = ''
    vSpawnParams = []
    vargv = sys.argv
    vPID = '-1'
DefaultFileBase = 'vsgOut_'+textrip(vargv)+'_'+vshortnow
if vSpawned:
    DefaultFileBase = 'vsgOut_'+vsgid+'_'+str(vPID)+'_'+vshortnow
...
vDefaultLogFileName0 = vDefaultLogFileName
if vSpawned:
    vDefaultLogFileName = 'VSGLog_'+vsgid+'_'+str(vPID)+'_'+vshortnow+'.txt'
...
class VSGcanvas(list):
    ...
    def __init__(self,**d3):
        ...
        self.screenwidth=1920  ## Maximum width for display of output in browser (svg or html) or iPython Notebook 
        ...
        ## display defaults
        self.live=False   ## update canvas after each object is added.
...
##                    if s.vjust=='bottom': GMcvgcycorr-=s.cvxr
    ...
    def SVGwrite(self,filepath=DefaultFileBase+'.svg',display=True):
        ...
        if self.scale in ('auto','Auto'):
            if abs(self.xmax-self.xmin)>4*self.screenwidth:
                if self.xmax==self.xmin:
                    self.xmax=self.xmin+1
                sc=float(4*self.screenwidth)/abs(self.xmax-self.xmin)
        ...
        svgheader += '<svg width="'+str(int(self.cvwidth))+'px" height="'+str(int(self.cvheight))+'px"\n'
        ...
        for s in self:
            ...
            if s.popup!='':
                ...
                outline1 += ' onclick="alert('+"'"+vvsvge(s.detail)+"')"+'" /'
            if s.xlink!='':
                outline1='<a href="'+vvsvge(s.xlink)+'" >'+outline1+'></a'
            if s.mouseover!='':
                outline1='<a>'+outline1+'> <title>'+vvsvge(s.mouseover)+'</title></a'
...
##        """adds a line to canvas"""
    ...
    def vtitle(self,*dz0,**dz2):
        ...
        maxhorizfont = int((self.xmax-self.xmin)/(len(dz2['text'])+1))
        fs1=min(maxhorizfont,int(max(abs(self.xmax-self.xmin)/30+1,abs(self.ymax-self.ymin)/30+1)))
        if not('font' in dz2):
    ...
    def vcolorkey(self,*dz0,**xgd):
        ...
        if "keyheight" in d:
            ...
            mindim=min((self.ymax-self.ymin)//12,abs(d['x2']-d['x'])//24)
            keyheight=mindim
        if 'ckfont' in d:
    ...
    def __init__(s,parentcanvas,*pr1,**d1):
        ...
        if s.obj=='text':
            ...
            if (s.metric.lower()=='pil' and not(Imageimported1)) or (s.metric.lower()=='lastditch'):
                ...
                d2['width']=max([len(myline1) for myline1 in multilinetext1])*s.font['size']*1.2/AveWidRatio1
...
def vhelp():
    ...
    for h1 in open(sys.argv[0],mode='r').readlines():
        ...
        if 'end help' in h1.lower(): break
        if not(h1.strip().startswith('#')) and (h1.strip().endswith(':')): break
        print(h1.rstrip().strip('#'))
    print('* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * ')
    print()
            
vv_argv = []
vv_lowerarg = map(lambda x:x.lower(), vargv)
if 'vcommand' in vv_lowerarg:
    vv_argv = vargv[vv_lowerarg.index('vcommand'):]
else:
    try:
...
if vv_argv:
    ...
    for vv_a1 in vv_argv:
        if 'help'==vv_a1.lower() or vv_a1.lower()=='-h' or vv_a1.lower()=='h':
            ...
            raise Exception("\rMinimal Help is Above; Further documentation in the source code file: "+os.path.basename(sys.argv[0]))
        ...
        if vv_VarMnemonic in vv_myvar1:
            ...
            vLog("Unknown Variable Name in Command Line Item: "+vv_a1)
            sys.tracebacklimit = 0
            raise Exception("\rERROR: Unknown Variable Name in Command Line Item "+vv_a1.split('=')[0].strip()+' ('+vv_a1+')')
        if '=' in vv_a1:
        ...
        if not(vSpawned): print("Setting "+vv_VarName+'='+str(vv_NewVarValue))
    ...
    for vv_vname in vv_myvar0:
        ...
        vSysLogInfo1 += '\n'
    if vSpawned:
        vSysLogInfo1 = 'Start SubProcess '+vplusone(vPID)+'/'+vPMax+':  '+vSpawnFunction+'('+', '.join(vSpawnParams)+')'
    vLog(vSysLogInfo1)
# 
# UID in vsg (default when vnow is determined, then passed to child processes through vsgUID=xxx command line preference
# vcommand print statements should skip lines with code that is indented or definitions, so subroutine definitions can precede VCommand statement
# 
# 
# vShare(data,uid,descriptive)
# data = vGrab(uid,descriptive)
# vProcess = vProcess(Subroutine,paramaterlist)
# vprocess.spawn
# vprocess.done
# vProcess.harvest
# vPool([List of Processes], cores
# vHarvest(List of Processes IDs])


class vProcess():
    def __init__(self,Subroutine,ParameterList,myvPID):
        self.s = Subroutine
        self.p = ParameterList
        self.i = str(myvPID)
    def spawn(self,vPMax1):
        if vSpawned:
            print('')
            print('*********** Severe Error: Recursive Spawning is not supported in VSG. Terminating SubProcess ***********')
            print('')
            exit()
        subprocess.Popen([sys.executable,]+sys.argv+["spawned",str(vsgid),self.i,str(vPMax1),vnow,vshortnow,str(vt0),self.s]+list(map(str,self.p)), env=os.environ)
    def test(self):
        if os.path.isfile('vsgReturns_'+vsgid+'_'+self.i+'_'+vshortnow+'.done'):
            return True
        else:
            return False
    def harvest(self):
        myF = 'vsgReturns_'+vsgid+'_'+self.i+'_'+vshortnow+'.pck'
        pf = open(myF, mode='rb')
        pp = pickle.Unpickler(pf)
        result = pp.load()
        pf.close()
        os.remove(myF)
        os.remove('vsgReturns_'+vsgid+'_'+self.i+'_'+vshortnow+'.done')
        return result
    def feed(self,myfeed):
        myF = 'vsgFeed_'+vsgid+'_'+self.i+'_'+vshortnow+'.pck'
        pf = open(myF, mode='wb')
        pp = pickle.Pickler(pf)
        pp.dump(myfeed)
        pf.close()
        open('vsgFeed_'+vsgid+'_'+self.i+'_'+vshortnow+'.ready',mode='wt').write('ready')
        return 0
    def cleanup(self):
        myLog = vLog(open('VSGLog_'+vsgid+'_'+self.i+'_'+vshortnow+'.txt',mode='rt').read(),'vsilent')
        os.remove('VSGLog_'+vsgid+'_'+self.i+'_'+vshortnow+'.txt')
        return myLog
def vCleanup(descriptor):
    os.remove('vsgData_'+vsgid+'_'+descriptor+'_'+vshortnow+'.pck')
    os.remove('vsgData_'+vsgid+'_'+descriptor+'_'+vshortnow+'.done')

def vEat(MaxWaitSeconds=600):
    myF = 'vsgFeed_'+vsgid+'_'+str(vPID)+'_'+vshortnow+'.pck'
    waitcount = 0
    while not(os.path.isfile('vsgFeed_'+vsgid+'_'+str(vPID)+'_'+vshortnow+'.ready')):
        sleep(0.1)
        if not(os.path.isfile(vDefaultLogFileName0)):
            raise Exception('Derivative Process VSG_'+vsgid+'_'+str(vPID)+'_'+vshortnow+' terminated due to absence of parent VSGLog file '+vDefaultLogFileName0)
        if waitcount > MaxWaitSeconds*10:  #Failsafe that ensures that processes are terminated within 10 minutes of wait time for new task
            raise Exception('Derivative Process VSG_'+vsgid+'_'+str(vPID)+'_'+vshortnow+' terminated due to exceeded wait time for parent '+vDefaultLogFileName0)
        waitcount += 1   
    os.remove('vsgFeed_'+vsgid+'_'+str(vPID)+'_'+vshortnow+'.ready')
    pf = open(myF, mode='rb')
    pp = pickle.Unpickler(pf)
    result = pp.load()
    pf.close()
    os.remove(myF)
    return result
def vShare(data,descriptor):
    myF = 'vsgData_'+vsgid+'_'+descriptor+'_'+vshortnow+'.pck'
    pf = open(myF, mode='wb')
    pp = pickle.Pickler(pf)
    pp.dump(data)
    pf.close()
    open('vsgData_'+vsgid+'_'+descriptor+'_'+vshortnow+'.done',mode='wt').write('done')
def vSubRun(data):
    myF = 'vsgReturns_'+vsgid+'_'+str(vPID)+'_'+vshortnow+'.pck'
    pf = open(myF, mode='wb')
    pp = pickle.Pickler(pf)
    pp.dump(data)
    pf.close()
    ##vLog('Finished SubProcess '+vplusone(vPID)+'/'+vPMax+':  '+vSpawnFunction+'('+', '.join(vSpawnParams)+')')
    open('vsgReturns_'+vsgid+'_'+str(vPID)+'_'+vshortnow+'.done',mode='wt').write('done')
    quit()
def vDeposit(data):
    myF = 'vsgReturns_'+vsgid+'_'+str(vPID)+'_'+vshortnow+'.pck'
    pf = open(myF, mode='wb')
    ...
    ##vLog('Finished SubProcess '+vplusone(vPID)+'/'+vPMax+':  '+vSpawnFunction+'('+', '.join(vSpawnParams)+')')
    open('vsgReturns_'+vsgid+'_'+str(vPID)+'_'+vshortnow+'.done',mode='wt').write('done')
def vGrab(descriptor,MaxWaitSeconds=600):
    while not(os.path.isfile('vsgData_'+vsgid+'_'+descriptor+'_'+vshortnow+'.done')):
        sleep(0.02)
        waitcount = 0
        if not(os.path.isfile(vDefaultLogFileName0)):
            raise Exception('Derivative Process VSG_'+vsgid+'_'+str(vPID)+'_'+vshortnow+' terminated due to absence of parent VSGLog file '+vDefaultLogFileName0)
    ...
    pf.close()
    return result
def vPoolEx(vProcessList,cores=os.cpu_count()):
    Results = [0] * len(vProcessList)
    ProcessStatus = [0]*len(vProcessList)  ## 0 for not run, 1 for running, 2 for complete
    while True:
        for i,p in enumerate(vProcessList):
            if ProcessStatus[i]==1 and p.test():
                ProcessStatus[i] = 2
                Results[i] = p.harvest()
            elif ProcessStatus[i]==0 and ProcessStatus.count(1)<cores:
                ProcessStatus[i] = 1            
                p.spawn(len(vProcessList))
        sleep(0.02)
        if min(ProcessStatus)==2:
            break
    return Results

## A few useful molecular Biology Routintes

def vFastAToDict(Fn, upper=False, lower=False, stripN=False, idwords='',OneReference=False):
    ''' takes a FastA file or file name and returns a dictionary where the keys are sequence ids and the values are sequend
        ...
        OneReference instructs the program to return a single sequence for a file that may contain many contigs.
        If OneReference is a string, then this will be the name of the sequence.  If OneReference is "True" then a name is assigned'''
    if not(type(Fn)==str):
        F = Fn
    ...
    for f in F0:
        try:
            ...
            pass
    if OneReference:
        if OneReference==True:
            if len(D)==1:
                myName = list(D.keys())[0]
            else:
                myName = os.path.basename(F.name)
        else:
            myName = OneReference
        myDelimiter = 'N'
        if lower:
            myDelimiter = 'n'        
        D = {myName:myDelimiter.join(D.values())}
    return D
if not(vSpawned): print('Successful import of VSG Version='+VSGVersion)
